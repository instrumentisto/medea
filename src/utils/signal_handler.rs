...
use actix_rt::spawn;
use actix::prelude::{Actor, Recipient, Message};
            ...
            #[cfg(not(unix))]
                {
                    tokio_signal::ctrl_c()
                        .map_err(|_| ())
                        .and_then(move |stream| SignalHandler {
                            srv,
            ...
            #[cfg(unix)]
                {
                    ...
                    let mut sigs: Vec<Box<Future<Item = SigStream, Error = io::Error>>> = Vec::new();
                    sigs.push(Box::new(
                        tokio_signal::unix::Signal::new(tokio_signal::unix::SIGINT).map(|stream| {
                            let s: SigStream = Box::new(stream.map(|_| SignalKind::Int));
                            s
                        }),
                    ));
                    sigs.push(Box::new(
                        tokio_signal::unix::Signal::new(tokio_signal::unix::SIGHUP).map(
                            |stream: unix::Signal| {
                                let s: SigStream = Box::new(stream.map(|_| SignalKind::Hup));
                                s
                            },
                        ),
                    ));
                    sigs.push(Box::new(
                        tokio_signal::unix::Signal::new(tokio_signal::unix::SIGTERM).map(
                            |stream| {
                                let s: SigStream = Box::new(stream.map(|_| SignalKind::Term));
                                s
                            },
                        ),
                    ));
                    sigs.push(Box::new(
                        tokio_signal::unix::Signal::new(tokio_signal::unix::SIGQUIT).map(
                            |stream| {
                                let s: SigStream = Box::new(stream.map(|_| SignalKind::Quit));
                                s
                            },
                        ),
                    ));
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        #[cfg(not(unix))]
            ...
            match self.stream.poll() {
                ...
                Ok(Async::Ready(Some(sig))) => self.srv.do_send(SignalMessage(sig)),
