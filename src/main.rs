...
fn main() -> Result<(), Error> {
    ...
    //    }
    //
    //    let logger = log::new_dual_logger(std::io::stdout(),
    // std::io::stderr());    let _scope_guard =
    // slog_scope::set_global_logger(logger);    slog_stdlog::init()?;
    //
    //    info!("{:?}", config);
    //
    //    actix::run(move || {
    ...
    //                        );
    //
    //                        let server = Server::run(room_repo,
    // config).unwrap();                        shutdown::subscribe(
    //                            &graceful_shutdown,
    //                            server.recipient(),
    //                            shutdown::Priority(1),
    //                        );
