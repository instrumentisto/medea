...
/// Component that manages [`MediaExchangeState`].
pub struct MediaExchangeStateController {
    /// Actual [`MediaExchangeState`].
    state: ObservableCell<MediaExchangeState>,
...
impl MediaExchangeStateController {
    ...
    pub(in super::super) fn new(state: StableMediaExchangeState) -> Rc<Self> {
    ...
    pub fn media_exchange_state(&self) -> MediaExchangeState {
        self.state.get()
    }
