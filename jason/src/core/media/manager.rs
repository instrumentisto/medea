...
impl InnerMediaManager {
    ...
    /// Retrieves tracks from provided [`platform::MediaStream`], saves tracks
    /// weak references in [`MediaManager`] tracks storage.
    ///
    /// # Errors
    ///
    /// With [`MediaManagerError::LocalTrackIsEnded`] if at least one track from
    /// the provided [`platform::MediaStream`] is in [`ended`][1] state.
    ///
