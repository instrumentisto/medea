...
pub type ArrayCompleterComplete = extern "C" fn(Dart_Handle, var: PtrArray);
static mut ARRAY_COMPLETER_COMPLETE: Option<ArrayCompleterComplete> = None;

#[no_mangle]
pub unsafe extern "C" fn register_array_completer_complete(
    f: ArrayCompleterComplete,
) {
    ARRAY_COMPLETER_COMPLETE = Some(f);
}
...
impl<E: 'static> Completer<PtrArray, E> {
    pub fn complete_with_array(&self, arg: PtrArray) {
        unsafe {
            let handle = Dart_HandleFromPersistent_DL_Trampolined(self.handle);
            ARRAY_COMPLETER_COMPLETE.unwrap()(handle, arg);
        }
    }
}
