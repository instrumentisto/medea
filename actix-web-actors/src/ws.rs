    ...
    A: Actor<Context = Self>,
{
    ...
    #[inline]
    pub fn flushes_count(&self) -> usize {
        self.flushes_count
    }

    #[inline]
    pub fn flushed_messages_count(&self) -> usize {
        self.flushed_messages_count
    }

    #[inline]
    pub fn last_flush_time(&self) -> SystemTime {
        self.last_flush_time
    }

    #[inline]
    pub fn queued_messages_count(&self) -> usize {
        self.messages.len()
    }
    ...
    A: Actor<Context = WebsocketContext<A>>,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        this.fut.ctx().last_flush_time = SystemTime::now();
        let count_of_messages = this.fut.ctx().messages.len();
        let last_flushed_messages_count = this.fut.ctx().flushed_messages_count;
        this.fut.ctx().flushed_messages_count = last_flushed_messages_count + count_of_messages;
        this.fut.ctx().flushes_count += 1;
        // encode messages
